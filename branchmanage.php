...
<?php
 ...
 $query2=oci_parse($conn,"select * from available_car");
 $query_result2=oci_execute($query2);
 $query1=oci_parse($conn,"select * from branch_restock");
...
<p class="logo"><span>A</span>dmin</p>
	...
	<span style="font-size:30px;cursor:pointer; color: white;" class="nav2"  >☰ Dashboard</span>
		...
		<br><br><br>
    <h2 style="color:aliceblue"><center>branch details</center></h2>
	
				<table>
	  <tr>
	    <th>Branch Id</th>
		<th>Branch available car </th>
	...
	<tr>
		...
		<td><?php if(isset($row[5]))
			{echo $row[5];} ?></td>
			<td><form action="updatebranch.php" method="post">
		<input type="text" name="car_number" placeholder="update available car"/>

<input type="hidden" name="update_id" value="<?php echo $row['BRANCH_ID']; ?>">

<button type="submit" name="update_p_btn" class="btn btn-success"> Update</button>
	...
	<h2 style="color:aliceblue"><center> branch car restock</center></h2>
